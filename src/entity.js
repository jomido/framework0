...
const make = (context) => {
    ...
    const setComponent = (o, state, failIfNew=false) => {
        ...
        let component = context.Component.toComponent(state)
        let componentName = ComponentType.check(component)
    ...
    const setComponentByString = (entity, pathValue) => {
        ...
        let instance = context.Component.get(componentName)()
        ...
        for (let path of paths) {
            ...
            if (isObject(value)) {
                if (pathParts.length === 1)  { // i don't like this
                    clone = context.Component.get(componentName)(value)()  // returns a clone
                }
                else {
                ...
                if (pathParts.length === 1)  { // i don't like this
                    clone = context.Component.get(componentName)(value)()
                }
                else {
                    clone = merge([], value, {clone: true})
                }
    ...
    const Entity = (name, ...components) => {
        ...
        context.registry[name] = EntityFunction
    ...
    Entity.get = (name) => context.registry[name]

    Entity[Symbol.iterator] = getIterator(() => getValues(context.registry))

    return Entity
}

const Entity = make({registry, events, Component})
